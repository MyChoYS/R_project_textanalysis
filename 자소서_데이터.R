...
remDr$open()
#링커리어 합격 자소서 긁어모으기 #데이터 직무별 긁어오기
remDr$navigate("https://linkareer.com/cover-letter/26391?page=1&role=%EB%8D%B0%EC%9D%B4%ED%84%B0&sort=RELEVANCE&tab=all")
all_comname <- NULL
all_spec <- NULL
act_spec <- NULL
...
write.csv(all_comname, "output/all_comname.csv")
write.csv(all_spec, "output/all_spec.csv")
write.csv(act_spec, "output/act_spec.csv")
